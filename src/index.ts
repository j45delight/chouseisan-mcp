...
import type { CreateEventRequest } from "./types/index.js";
    ...
    this.server.registerTool(
      "create_chouseisan_event",
      {
        ...
        inputSchema: {
          ...
          schedule: z.string().describe("日程の指定（自然言語）。例: '毎週金曜日', '1月15日、1月22日、1月29日', '毎週月水金', '1月から3月まで毎週火曜日'"),
          timeFormat: z.string().optional().default("19:30〜").describe("時間帯の表記（例: '19:30〜', '10:00～12:00'）"),
          memo: z.string().optional().describe("メモや説明文")
        }
      },
      async (request) => {
        try {
          const { title, schedule, timeFormat = "19:30〜", memo } = request as CreateEventRequest;

          // 自然言語による日程解析
          console.error(`日程解析開始: ${schedule}`);
          const dateCandidates = DateCalculator.parseSchedule(schedule, { timeFormat });
    ...
    this.server.registerTool(
      "preview_schedule_candidates",
      {
        ...
        inputSchema: {
          ...
          timeFormat: z.string().optional().default("19:30〜").describe("時間帯の表記"),
          maxDates: z.number().optional().default(10).describe("表示する最大日程数")
        }
      },
      async (request) => {
        try {
