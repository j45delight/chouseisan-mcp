...
export class ImprovedDateCalculator {
     ...
     * 除外日付を抽出
     */
    static extractExcludedDates(scheduleText) {
        const exclusions = [];
     ...
     * 除外指定のテキストを除去
     */
    static removeExclusionText(scheduleText) {
        return scheduleText
     ...
     * 除外日付を適用
     */
    static applyExclusions(candidates, exclusions) {
        if (exclusions.length === 0)
            return candidates;
        return candidates.filter(candidate => {
            return !exclusions.some(exclusion => candidate.date.getFullYear() === exclusion.getFullYear() &&
     ...
     * 指定月の最初の該当曜日を探す
     */
    static findFirstMatchingDayInMonth(monthStart, daysOfWeek) {
     ...
     * 終了日を計算
     */
    static calculateEndDate(startDate, patterns, explicitEndDate) {
        if (explicitEndDate)
...
//# sourceMappingURL=improved-date-calculator.js.map